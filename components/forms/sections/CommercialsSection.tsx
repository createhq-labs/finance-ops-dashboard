...
export function CommercialsSection({ values, totalAmount, onChange, errors = {} }: Props) {
  ...
  return (
    <section className="intake-section">
      ...
      >
        <style>{COMMERCIALS_MEDIA_CSS}</style>
        <div className={isInfluencerMarketing ? "commercials-grid-im" : "commercials-grid-tm"} style={{ display: "grid", gap: 12, alignItems: "start" }}>
          <label className="intake-field">
            <span className="intake-label">
              {isInfluencerMarketing ? "Deal Amount (" + currencyLabel + ") *" : "Total Amount (" + currencyLabel + ") *"}
            </span>
            <input
          ...
          <label className="intake-field">
            <span className="intake-label">Additional Agency Commission ({currencyLabel})</span>
            <input
              className="intake-input"
              type="number"
              inputMode="decimal"
              ...
              autoComplete="off"
              placeholder={"Commission (" + currencyLabel + ")"}
            />
            <div style={{ minHeight: 16 }} />
          </label>
          ...
          {isInfluencerMarketing ? (
            <label className="intake-field">
              <span className="intake-label">Total/Gross Amount ({currencyLabel})</span>
              <input
                className="intake-input"
                type="text"
                value={totalAmount}
